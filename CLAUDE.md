...
**`Net.Proto.TcpClientStateMachine`** (`Assets/Scripts/Net/Proto/`): a separate FSM specifically for the TCP connection lifecycle. States live under `Net.Proto.State` (`Init → Connecting → Handshaking → Connected → Ready → ... → Disconnecting → Disconnected → Closed`, with `Reconnecting`). Each state is a singleton cached in a static dictionary (`TcpClientStateBase.GetInstance<T>()`) and operates on an `ITcpClientFSMCtx`. **Note**: `GameTcpClient` itself currently has its own inline `ETcpConnectionState` field and `ChangeState()` calls — the `TcpClientStateMachine` exists in parallel and is only partially wired up (e.g., `Connecting.RunAsyncInternal` still has a `// StartLoop(ct); // TODO`). Don't assume it's the live state machine when reading `GameTcpClient.cs`. Outstanding refactor work is tracked in `TODO.md`.

### Networking (`Assets/Scripts/Net/`)

- **Wire format**: `[Magic 4B = 0xCAFEBABE][Type 2B][Length 4B][Body N B][CRC32 4B]`, all big-endian. `MAX_BODY_SIZE = 1 MB`. Defined in `GameTcpClient.SendMessage` / `ParseOther`.
...
- **Async model**: everything is **UniTask**, not `Task`. Two long-running loops are started after connect: `ListenLoopAsync` (read socket → push into `RingBufferStream`) and `ParseLoopAsync` (consume frames → dispatch). Cancellation flows via a single `CancellationTokenSource` owned by `GameTcpClient`.
